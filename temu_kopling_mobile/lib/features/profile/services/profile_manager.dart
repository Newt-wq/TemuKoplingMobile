...
class ProfileManager extends ChangeNotifier {
  ...
  /// Mendekode string base64 atau URL menjadi ImageProvider secara aman (try-catch)
  static ImageProvider? getProfileImage(String? source) {
    if (source == null || source.trim().isEmpty) return null;
    
    final cleanSource = source.trim();
    
    // Jika source adalah URL jaringan
    if (cleanSource.startsWith('http://') || cleanSource.startsWith('https://')) {
      return NetworkImage(cleanSource);
    }

    try {
      String cleanBase64 = cleanSource;
