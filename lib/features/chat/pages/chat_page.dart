...
class ChatPage extends StatefulWidget {
  ...
  final ValueChanged<bool>? onChatSessionActive;
  const ChatPage({super.key, this.newRiderInfo, this.onChatSessionActive});
...
class _ChatPageState extends State<ChatPage> {
  ...
  void didUpdateWidget(ChatPage oldWidget) {
    ...
    // Jika menerima rider info baru dari Tracking Page (timestamp/reference berbeda)
    if (widget.newRiderInfo != null && widget.newRiderInfo != oldWidget.newRiderInfo) {
      _handleNewRiderInfo(widget.newRiderInfo!);
    }
  }

  void _handleNewRiderInfo(Map<String, dynamic> rider) {
    ...
    if (_customerId != null && riderIdStr.isNotEmpty) {
      ...
      if (existingIndex != -1) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          widget.onChatSessionActive?.call(true);
          _scrollToBottom();
        });
      } else {
  ...
  void _deleteSession(ChatSession session) {
    setState(() {
      ...
      if (_selectedSessionId == session.id) {
        ...
        widget.onChatSessionActive?.call(false);
      }
    });
  }
